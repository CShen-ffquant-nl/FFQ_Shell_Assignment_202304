"""
Implementation for API for durable function

NOTE the functions may be called by name in function_app.py. so be careful when change the function name.  
"""
...
"""
Implementation for upload market data API
"""
...
"""
Implementation for upload calibration data API
"""
...
"""
Implementation for upload and calculate API
"""
...
def calculate_main(input: tuple[dict, str]) -> dict:
    try:
        ...
        for aContract in output:
            ...
            filename = max(
                blob_helper.get_container("marketdata").list_blob_names(name_starts_with=underlying_name)
            )  # choose the latest file name
            ...
            filename = max(
                blob_helper.get_container("calibrationdata").list_blob_names(name_starts_with=scenario_name)
            )  # choose the latest file name
